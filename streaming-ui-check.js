...
event-interaction https://raw.githubusercontent.com/KOP-XIAO/QuantumultX/master/Scripts/streaming-ui-check.js, tag=流媒體-解鎖查詢, img-url=checkmark.seal.system, enabled=true
...
let result = {
  "title": '📺 流媒體服務查詢',
  "YouTube": 'YouTube: 檢測失敗，請重試',
  "Netflix": 'Netflix: 檢測失敗，請重試',
  "Dazn": "Dazn: 檢測失敗，請重試",
  "Disney": "Disney: 檢測失敗，請重試",
...
function testYTB() { 
    ...
    $task.fetch(option).then(response=> {
      ...
      if (response.statusCode !== 200) {
        //reject('Error')
        result["YouTube"] = "YouTube Premium: 檢測失敗"
      ...
      result["YouTube"] = "YouTube Premium: 檢測超時"
      //resolve("timeout")
    })
}
...
function testDisney(){
    ...
    $task.fetch(option).then(response => {
      ...
      if(response.statusCode === 200)
        {
          if(response.body.indexOf("Sorry, Star+ is not available in your region") !== -1) // 無服務
          {
          ...
          } else if(response.body.indexOf('<link rel="canonical" href="https://www.hotstar.com/') !== -1) // 印度東南亞 Hotstar 版
          {
            ...
            result["Disney"]= "Disney+: 即將登陸"+ "⟦"+flags.get(D_region.toUpperCase())+"⟧"+ " (Hotstar)";
          } else if(response.body.indexOf("CNBL: 1") !== -1) // 國際版
          {
            D_region = response.body.substring(response.body.indexOf("Region: ") + 8, response.body.indexOf("Region: ") + 10);
            result["Disney"]= "Disney+: 已支持"+ "⟦"+flags.get(D_region.toUpperCase())+"⟧"
          } else if(response.body.indexOf("CNBL: 2") !== -1) // 國際版 即將上線
          {
            D_region = response.body.substring(response.body.indexOf("Region: ") + 8, response.body.indexOf("Region: ") + 10);
            result["Disney"]= "Disney+: 即將登陸"+ "⟦"+flags.get(D_region.toUpperCase())+"⟧"
          }
          console.log(result["Disney"])
        }
    }, reason => {
      result["Disney"]= "Disney+: 檢測超時"
    })
}
